...
export default function ConsultEnClient() {
  ...
  const [loading,setLoading]=useState(false); const [error,setError]=useState(""); const [acceptedTerms,setAcceptedTerms]=useState(false); const [hydrated,setHydrated]=useState(false);
  useEffect(()=>{try{const raw=JSON.parse(sessionStorage.getItem("cs_intl_draft")||"null");if(raw){if(Number(raw.step)>=1&&Number(raw.step)<=5)setStep(Number(raw.step));if(typeof raw.category==="string")setCategory(raw.category);if(typeof raw.question==="string")setQuestion(raw.question);if(Array.isArray(raw.picked))setPicked(raw.picked.slice(0,3));if(typeof raw.name==="string")setName(raw.name);if(typeof raw.email==="string")setEmail(raw.email);if(raw.acceptedTerms===true)setAcceptedTerms(true);}}catch{}setHydrated(true);},[]);
  useEffect(()=>{if(!hydrated)return;sessionStorage.setItem("cs_intl_draft",JSON.stringify({step,category,question,picked,name,email,acceptedTerms}));},[hydrated,step,category,question,picked,name,email,acceptedTerms]);
  ...
  async function checkout(e:React.FormEvent){e.preventDefault();setError("");setLoading(true);try{
    ...
    const response=await fetch("/api/international/checkout",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,email,category,question,cardIds:picked,acceptedTerms,...context,...source})});
