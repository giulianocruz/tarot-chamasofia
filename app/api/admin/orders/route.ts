...
export async function GET(request: Request) {
  ...
  const [ordersResult, totals, today, events, funnel, paidFunnel, paidTraffic, campaignEventRows, ebookProductRows, campaignOrderRows, pricing, bookInventory, visitorTraffic, international, upsellInterest] = await Promise.all([
    getD1()
      .prepare(
  ...
  return Response.json(
    {
      ...
      dashboard: {
        ...
        pricing,
        traffic: {
          sessionsObserved: Number(visitorTraffic?.sessions || sessions),
          paidSessionsObserved: Number(visitorTraffic?.paid_sessions || paidSessions),
          lastVisit: visitorTraffic?.last_visit ? String(visitorTraffic.last_visit) : null,
          paidSessions,
          paidSales,
          paidRevenue,
