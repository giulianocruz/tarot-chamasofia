...
export default function PreviewDashboard({ cards, category, question, preview }: PreviewDashboardProps) {
  ...
  const indicators = [
    ["Clareza", clarity, "O quanto o jogo convida a enxergar fatos e padr?es."],
    ["Movimento", movement, "Quanto existe de impulso para sair do ponto atual."],
    ["Tens?o", tension, "Onde a leitura pede mais cuidado, pausa ou limite."],
    ["Autonomia", autonomy, "Quanto o pr?ximo passo depende de uma escolha sua."],
  ] as const;

  return (
    <section className="preview-dashboard" aria-label="Pr?via simb?lica da leitura">
      <div className="preview-dashboard-head">
        <div>
          <p className="eyebrow">Painel simb?lico ? pr?via da leitura</p>
          <h2>O desenho inicial das suas cartas</h2>
          <p className="preview-dashboard-sub">Uma primeira camada sobre <strong>{category.toLowerCase()}</strong>. A leitura completa conecta as tr?s cartas ? sua pergunta e aprofunda tend?ncia, conselho e s?ntese.</p>
        </div>
        <div className="preview-completion" style={{ "--preview-progress": "31%" } as React.CSSProperties}>
          <strong>31%</strong><span>da an?lise<br/>aberta</span>
        </div>
      </div>
      ...
      <div className="preview-focus-grid">
        <article className="preview-open-insight">
          ...
          <div className="preview-open-card">
            ...
            <div><small>SITUA??O ATUAL</small><h3>{cards[0]?.name}</h3><p>{preview}</p></div>
          </div>
        </article>
        <article className="preview-question-card">
          ...
          <p>As outras duas cartas mudam o sentido desta primeira impress?o quando s?o lidas em conjunto.</p>
        </article>
      </div>

      <div className="preview-locked-zone">
        <div className="preview-locked-title"><img src="/assets/tarot/ui/insight-lock.svg" alt=""/><div><span>Na leitura completa</span><strong>4 camadas ainda ser?o conectadas</strong></div></div>
        <div className="preview-locked-grid">
          {["Influ?ncia da segunda carta", "Tend?ncia e conselho da terceira", "Como as 3 cartas conversam", "S?ntese + reflex?o final no PDF"].map((item) => <span key={item}>? {item}</span>)}
        </div>
      </div>
    </section>
  );
